...
def createapp():  
    ...
    app=Flask(__name__,instance_relative_config=True)#This will eable line 11 to know that it needs to go amd load config
    #from skinapp import config
    app.config.from_pyfile("config.py",silent=True) 
    from skinapp.models import db
    db.init_app(app) 
    csrf.init_app(app)
...
app = createapp()
from skinapp import user_routes,admin_routes,forms, models
